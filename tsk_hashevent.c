...
char hash_start_string[] = "{H";
char hash_end_string[] = "}";
...
TASK(HASHTASK) 
{
	...
	while(1) {
        ...
        //for(j = 0; j < 2; j++){
        //    send_buffer[send_place_to_write++%32] = hash_start_string[j];
        //}
        for(j = 0; j < 19; j++){
            send_buffer[send_place_to_write++%32] = test[j];
        }
        //send_buffer[send_place_to_write++%32] = hash_end_string[0];
